...
/**
 * ==============================================================================
 * This file will have the configuration related to price tab only at admin panel.
 * ==============================================================================
 */
...
// remove action hook.
remove_action( 'wp_travel_tabs_content_' . WP_TRAVEL_POST_TYPE . '_price', 'wp_travel_trip_callback_price', 12 );

/**
 * Function to create custom template for Price-tab.
 *
 * @param string $tab tab name.
 */
function wp_travel_tuskphoto_custom_price_tab_callback( $tab ) {
	if ( 'price' !== $tab ) {
		return;
	}
	...
	// custom
	// $enable_additional_currency_sale = get_post_meta ( $post_id , 'wp_travel_additional_currency_enable_sale' , true );
	$additional_currency_attribute = 'disabled="disabled"';
	$additional_currency_class = 'hidden';

	// if ( $single_enable_additional_currency ) {
	// 	$additional_currency_attribute = '';
	// 	$additional_currency_class = '';
	// }

	$enable_additional_currency_sale = get_post_meta ( $post_id, 'single_enable_additional_currency_sale' , true );
	$additional_currency_sale_amount = get_post_meta ( $post_id , 'additional_currency_sale_price' , true);
	...
	$single_enable_additional_currency = get_post_meta( $post_id, 'single_enable_additional_currency', true );
	$single_select_additional_currency = get_post_meta( $post_id, 'single_select_additional_currency', true );
	$single_additional_currency_amount = get_post_meta( $post_id, 'single_additional_currency_amount', true );
	
	// if ( is_array( $trip_pricing_options_data ) && '' !== $trip_pricing_options_data ) :
	// 	foreach ( $trip_pricing_options_data as $single_price_key => $single_pricing ) {

	// 		// Additional currency.
	// 		// $single_enable_additional_currency      = ! empty( $single_pricing['single_enable_additional_currency'] ) ? $single_pricing['single_enable_additional_currency'] : '';
	// 		// $single_select_additional_currency      = ! empty( $single_pricing['single_select_additional_currency'] ) ? $single_pricing['single_select_additional_currency'] : '';
	// 		// $single_additional_currency_amount      = ! empty( $single_pricing['single_additional_currency_amount'] ) ? $single_pricing['single_additional_currency_amount'] : '';
	// 		$singel_sale_additional_currency_amount = ! empty( $single_pricing['single_sale_additional_currency_amount'] ) ? $single_pricing['singel_sale_additional_currency_amount'] : '';			
	// 		// echo '<pre>';
	// 		// print_r($trip_pricing_options_data);
	// 		// echo '<pre>';
	...
	<table class="form-table pricing-tab">
		...
		<!-- Custom -->
		<tr class="price-option-row <?php echo esc_attr( $single_pricing_option_class ); ?> checkbox-style">
			<td><label for="wp-travel-enable-additional-currency"><?php esc_html_e( 'Enable Additional Currency', 'wp-travel' ); ?></label></td>
			<td>
				<span class="show-in-frontend checkbox-default-design">
					<label data-on="ON" data-off="OFF">
						<input name="single_enable_additional_currency" type="checkbox" id="single-enable-additional-currency" <?php checked( $single_enable_additional_currency, 'yes' ); ?> value="yes" />
						<span class="switch"></span>
					</label>
				</span>
				<p class="wp-travel-enable-additional-currency description"><?php esc_html_e( 'Check to enable additional currency', 'wp-travel' ); ?></p>
			</td>
		</tr>
		
		
		<tr class="price-option-row <?php echo esc_attr( $single_pricing_option_class ); ?> currency-options">
			<td><label for="wp-travel-enable-additional-currency select-currency"><?php esc_html_e( 'Select Currency', 'wp-travel' ); ?></label></td>
			<td>
				<span class="wp-travel-enable-additional-currency currency-options">
				<select name="single_select_additional_currency" id="" >
						<option value="USD" <?php selected( $single_select_additional_currency, 'USD' ); ?> ><?php esc_html_e( 'US', 'wp-travel' ); ?> ( &#36; )</option>
						<option value="CAD" <?php selected( $single_select_additional_currency, 'CAD' ); ?> ><?php esc_html_e( 'C$', 'wp-travel' ); ?> ( &#36; )</option>
						<option value="GBP" <?php selected( $single_select_additional_currency, 'GBP' ); ?> ><?php esc_html_e( 'UK', 'wp-travel' ); ?> ( &#163; )</option>
						<option value="EUR" <?php selected( $single_select_additional_currency, 'EUR' ); ?> ><?php esc_html_e( 'EU', 'wp-travel' ); ?> ( &#128; )</option>
						<option value="BWP" <?php selected( $single_select_additional_currency, 'BWP' ); ?> ><?php esc_html_e( 'Botswana Pula', 'wp-travel' ); ?> ( P )</option>
						<option value="ZAR" <?php selected( $single_select_additional_currency, 'ZAR' ); ?> ><?php esc_html_e( 'South Africa Rand', 'wp-travel' ); ?> ( Rand )</option>
						<option value="BRL" <?php selected( $single_select_additional_currency, 'BRL' ); ?> ><?php esc_html_e( 'Brazilian Real', 'wp-travel' ); ?> ( R&#36; )</option>
						<option value="JPY" <?php selected( $single_select_additional_currency, 'JPY' ); ?> ><?php esc_html_e( 'Japanese Yen', 'wp-travel' ); ?> ( &#165; )</option>
				</select>
				</span>
				<p class="wp-travel-enable-additional-currency description"><?php esc_html_e( 'Select a Currency', 'wp-travel' ); ?></p>
			</td>
		</tr>

		<tr class="price-option-row <?php echo esc_attr( $single_pricing_option_class ); ?> currency-amount">
			<td><label for="wp-travel-enable-additional-currency currency-amount"><?php esc_html_e( 'Additional Currency Amount', 'wp-travel' ); ?></label></td>
			<td><input name="single_additional_currency_amount" class="wp-travel-additional-currency-field" type="number" min="0" step="0.01"  value="<?php echo esc_attr( $single_additional_currency_amount ); ?>"/></td>
		</tr>

		<!-- Custom Sale enable -->
		<tr class="price-option-row <?php echo esc_attr( $single_pricing_option_class ); ?> checkbox-style">
			<td><label <?php echo esc_attr( $additional_currency_attribute ); ?>  for="wp-travel-enable-additional-currency-sale"><?php esc_html_e( 'Enable Sale for Additional Currency ', 'wp-travel' ); ?></label></td>
			<td>
				<span class="show-in-frontend checkbox-default-design">
					<label data-on="ON" data-off="OFF">
						<input name="single_enable_additional_currency_sale" type="checkbox" id="additional-currency-sale-price-hide" <?php checked( $enable_additional_currency_sale, 'yes' ); ?> value="yes" />
						<span class="switch"></span>
					</label>
				</span>
				<p class="wp-travel-enable-additional-currency-sale description"><?php esc_html_e( 'Check to enable "Additional Currency Sale"', 'wp-travel' ); ?></p>
			</td>
		</tr>
		<tr class="price-option-row <?php echo esc_attr( $single_pricing_option_class ); ?> <?php //echo esc_attr( $additional_currency_class ); ?>">
				<td><label for="additional-currency-sale-price"><?php esc_html_e( 'Additional Currency Sale Price', 'wp-travel' ); ?></label></td>
				<td><input name="additional_currency_sale_price" id="additional-currency-sale-price-hide" class="additional-currency-sale-price" type="number" min="0" step="0.01"  value="<?php echo esc_attr( $additional_currency_sale_amount ); ?>"/></td>
		</tr>
					...
					<div id="price-accordion" class="tab-accordion price-accordion wp-travel-accordion has-handler">
							...
							if ( is_array( $trip_pricing_options_data ) && '' !== $trip_pricing_options_data ) :
								foreach ( $trip_pricing_options_data as $key => $pricing ) {
									...
									// Additional currency.
									$enable_additional_currency      = ! empty( $pricing['enable_additional_currency'] ) ? $pricing['enable_additional_currency'] : '';
									$select_additional_currency      = ! empty( $pricing['select_additional_currency'] ) ? $pricing['select_additional_currency'] : '';
									$additional_currency_amount      = ! empty( $pricing['additional_currency_amount'] ) ? $pricing['additional_currency_amount'] : '';
									$enable_additional_currency_sale = ! empty ( $pricing['enable_additional_currency_sale'] ) ? $pricing ['enable_additional_currency_sale'] : '';
									$sale_additional_currency_amount = ! empty( $pricing['sale_additional_currency_amount'] ) ? $pricing['sale_additional_currency_amount'] : '';
									...
									<div id="collapse-<?php echo esc_attr( $key ); ?>" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading-<?php echo esc_attr( $key ); ?>">
										<div class="panel-body">
											...
											<div class="repeat-row">
												...
												<div class="repeat-row">
													<label for="wp-travel-enable-additional-currency_<?php echo esc_attr( $key ); ?>" class="one-third"><?php esc_html_e( 'Enable Additional Currency', 'wp-travel' ); ?></label>
													<div class="two-third">
														<span class="show-in-frontend checkbox-default-design">
															<label data-on="ON" data-off="OFF">
																<input name="wp_travel_pricing_options[<?php echo esc_attr( $key ); ?>][enable_additional_currency]" type="checkbox" class="wp-travel-enable-additional-currency" id="wp-travel-enable-additional-currency_<?php echo esc_attr( $key ); ?>" <?php checked( $enable_additional_currency, 'yes' ); ?> value="yes">
																<span class="switch"></span>
															</label>
														</span>
														<p class="description"><?php esc_html_e( 'Check to enable additional currency option.', 'wp-travel' ); ?></p>
													</div>
												</div>
											<!-- wrapper class -->
											<div class ="enable-additional-currency-wrapper" id="enable-additional-currency-wrapper">
												<div class="repeat-row wp-travel-additional-currency-hide-show">
													<label class="one-third"><?php esc_html_e( 'Select Currency', 'wp-travel' ); ?></label>
														<div class="two-third">
															<select name="wp_travel_pricing_options[<?php echo esc_attr( $key ); ?>][select_additional_currency]" id="">
																<option value="USD" <?php selected( $select_additional_currency, 'USD' ); ?>><?php esc_html_e( 'US', 'wp-travel' ); ?> ( &#36; )</option>
																<option value="CAD" <?php selected( $select_additional_currency, 'CAD' ); ?> ><?php esc_html_e( 'C$', 'wp-travel' ); ?> ( &#36; )</option>
																<option value="GBP" <?php selected( $select_additional_currency, 'GBP' ); ?>><?php esc_html_e( 'UK', 'wp-travel' ); ?> ( &#163; )</option>
																<option value="EUR" <?php selected( $select_additional_currency, 'EUR' ); ?>><?php esc_html_e( 'EU', 'wp-travel' ); ?> ( &#128; )</option>
																<option value="BWP" <?php selected( $select_additional_currency, 'BWP' ); ?>><?php esc_html_e( 'Botswana Pula', 'wp-travel' ); ?> ( P )</option>
																<option value="ZAR" <?php selected( $select_additional_currency, 'ZAR' ); ?>><?php esc_html_e( 'South Africa Rand', 'wp-travel' ); ?> ( R )</option>
																<option value="BRL" <?php selected( $select_additional_currency, 'BRL' ); ?>><?php esc_html_e( 'Brazilian Real', 'wp-travel' ); ?> ( R&#36; )</option>
																<option value="JPY" <?php selected( $select_additional_currency, 'JPY' ); ?>><?php esc_html_e( 'Japanese Yen', 'wp-travel' ); ?> ( &#165; )</option>
															</select>
															<p class="description"><?php esc_html_e( 'Select a currency.', 'wp-travel' ); ?></p>
														</div>
												</div>

												<div class="repeat-row wp-travel-additional-currency-hide-show">
													<label for="additional_currency_<?php echo esc_attr( $key ); ?>" class="one-third"><?php esc_html_e( 'Additional Currency Amount', 'wp-travel' ); ?></label>
													<div class="two-third">
														<div class="field-price-currency-input">
															<input id="additional_currency_<?php echo esc_attr( $key ); ?>" class="wp-travel-additional-currency-field" type="number" min="0" step="0.01" name="wp_travel_pricing_options[<?php echo esc_attr( $key ); ?>][additional_currency_amount]" value="<?php echo esc_attr( $additional_currency_amount ); ?>" <?php echo esc_attr( $enable_additional_currency == 'yes' ? 'required="required"' : '' ); ?>  />
														</div>
													</div>
												</div>
												...
												<div class="repeat-row">
													<label for="wp-travel-enable-additional-currency_sale<?php echo esc_attr( $key ); ?>" class="one-third"><?php esc_html_e( 'Enable Additional Currency Sale', 'wp-travel' ); ?></label>
													<div class="two-third">
														<span class="show-in-frontend checkbox-default-design">
															<label data-on="ON" data-off="OFF">
																<input name="wp_travel_pricing_options[<?php echo esc_attr( $key ); ?>][enable_additional_currency_sale]" type="checkbox" class="wp-travel-enable-additional-currency-sale" id="wp-travel-enable-additional-currency-sale_<?php echo esc_attr( $key ); ?>" <?php checked( $enable_additional_currency_sale, 'yes' ); ?> value="yes">
																<span class="switch"></span>
															</label>
														</span>
														<p class="description"><?php esc_html_e( 'Check to enable sale for additional currency price', 'wp-travel' ); ?></p>
													</div>
												</div>

												<div class="repeat-row wp-travel-sale-additional-currency-hide-show wp-travel-additional-currency-hide-show">
													<label for="sale_additional_currency_<?php echo esc_attr( $key ); ?>" class="one-third"><?php esc_html_e( 'Sale Price (Additional Currency)', 'wp-travel' ); ?></label>
													<div class="two-third">
														<div class="field-price-currency-input">
															<input id="sale_additional_currency_<?php echo esc_attr( $key ); ?>" class="wp-travel-sale-additional-currency-field" type="number" min="0" step="0.01" name="wp_travel_pricing_options[<?php echo esc_attr( $key ); ?>][sale_additional_currency_amount]" value="<?php echo esc_attr( $sale_additional_currency_amount ); ?>" />
															<p class="description"><?php esc_html_e( 'Enter the Sale Price for Additional Currency.', 'wp-travel' ); ?></p>
														</div>
													</div>
												</div>
											</div> <!-- closing div of additional currency wrapper -->
												<div class="repeat-row">
						...
						<div id="collapse-{{data.random}}" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="heading-{{data.random}}">
							<div class="panel-body">
								...
								<div class="repeat-row">
									...
									<div class="repeat-row">
										<label class="one-third"><?php esc_html_e( 'Enable Additional Currency', 'wp-travel' ); ?></label>
										<div class="two-third">
											<span class="show-in-frontend checkbox-default-design">
												<label data-on="ON" data-off="OFF">
													<input name="wp_travel_pricing_options[{{data.random}}][enable_additional_currency]" type="checkbox" class="wp-travel-enable-additional-currency" id="wp-travel-enable-additional-currency" value="yes">
													<span class="switch"></span>
												</label>
											</span>
											<p class="description"><?php esc_html_e( 'Check to enable additional currency option.', 'wp-travel' ); ?></p>
									...
									<div class="repeat-row wp-travel-additional-currency-hide-show">
										<label class="one-third"><?php esc_html_e( 'Select Currency', 'wp-travel' ); ?></label>
											<div class="two-third">
												<select name="wp_travel_pricing_options[{{data.random}}][select_additional_currency]" id="">
													<option value="USD"><?php esc_html_e( 'US', 'wp-travel' ); ?> ( &#36; )</option>
													<option value="GBP"><?php esc_html_e( 'UK', 'wp-travel' ); ?> ( &#163; )</option>
													<option value="EUR"><?php esc_html_e( 'EU', 'wp-travel' ); ?> ( &#128; )</option>
													<option value="BWP"><?php esc_html_e( 'Botswana Pula', 'wp-travel' ); ?> ( P )</option>
													<option value="ZAR"><?php esc_html_e( 'South Africa Rand', 'wp-travel' ); ?> ( R )</option>
													<option value="BRL"><?php esc_html_e( 'Brazilian Real', 'wp-travel' ); ?> ( R&#36; )</option>
													<option value="JPY"><?php esc_html_e( 'Japanese Yen', 'wp-travel' ); ?> ( &#165; )</option>
												</select>
												<p class="description"><?php esc_html_e( 'Select a currency.', 'wp-travel' ); ?></p>
											</div>
									</div>
									...
									<div class="repeat-row wp-travel-additional-currency-hide-show">
										<label for="additional_currency_{{data.random}}" class="one-third"><?php esc_html_e( 'Additional Currency Amount', 'wp-travel' ); ?></label>
										<div class="two-third">
											<div class="field-price-currency-input">
												<input id="additional_currency_{{data.random}}" class="wp-travel-additional-currency-field" type="number" min="0" step="0.01" name="wp_travel_pricing_options[{{data.random}}][additional_currency_amount]" />
											</div>
										</div>
									</div>
									...
									<div class="repeat-row wp-travel-additional-currency-hide-show">
										<label for="sale_additional_currency_{{data.random}}" class="one-third"><?php esc_html_e( 'Sale Price (Additional Currency)', 'wp-travel' ); ?></label>
										<div class="two-third">
											<div class="field-price-currency-input">
												<input id="sale_additional_currency_{{data.random}}" class="wp-travel-sale-additional-currency-field" type="number" min="0" step="0.01" name="wp_travel_pricing_options[{{data.random}}][sale_additional_currency_amount]" />
												<p class="description"><?php esc_html_e( 'Enter the Sale Price for Additional Currency.', 'wp-travel' ); ?></p>
											</div>
										</div>
									</div>
...
add_action( 'wp_travel_tabs_content_' . WP_TRAVEL_POST_TYPE . '_price', 'wp_travel_tuskphoto_custom_price_tab_callback', 20, 2 );
